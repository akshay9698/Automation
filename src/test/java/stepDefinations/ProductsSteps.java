...
public class ProductsSteps {
	...
	public void verify_their_prices_quantity_and_total_price(DataTable dataTable) {
		...
		for (int i = 0; i < expectedPrices.size(); i++) {
			Assert.assertEquals("Price of product " + (i + 1) + " should match.", expectedPrices.get(i),
					actualPrices.get(i));
		}

		// Verify Quantities
		for (int i = 0; i < expectedQuntys.size(); i++) {
			Assert.assertEquals("Quantity of product " + (i + 1) + " should match.", expectedQuntys.get(i),
					actualQuntys.get(i));
		}

		// Verify Total Prices
		for (int i = 0; i < expectedTotalPrices.size(); i++) {
			Assert.assertEquals("Total price of product " + (i + 1) + " should match.", expectedTotalPrices.get(i),
					actualTotalPrices.get(i));
		}
	...
	public void verify_product_detail_is_opened() {
		...
		Assert.assertTrue("Product details is not opened", pd.isProductDetailsVisible());

	}

	@When("Increase quantity to four and click add to cart button and view cart button")
	public void increase_quantity_to_four_and_click_add_to_cart_button_and_view_cart_button() {
	...
	public void verify_that_product_is_displayed_in_cart_page_with_exact_quantity() {
		...
		cp = new CartPage(BaseClass.getDriver());
		List<String> actQunty = cp.getProductsQnty();
		for (String quantity : actQunty) {
			// Parse the quantity string to an integer
			int actualQuantity = Integer.parseInt(quantity);

			// Assert that the quantity is 4
